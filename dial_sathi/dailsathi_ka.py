...
ASR_PIPELINE = pipeline(
    ...
    model="cdactvm/w2v-bert-kannada_new",
...
def reprocess_audio(input_filename, output_filename="processed.wav"):
    ...
    print(f"Audio reprocessed and saved as {output_filename}")
...
def text_to_speech(text, speaker_id = 9, style_id = 0, output_filename="output_kan.wav"):
    """
    ...
    # Save the audio file
    sf.write(output_filename, final_audio, model.config.sampling_rate)
...
def main():
    ...
    if not os.path.exists(raw_audio_file):
        ...
        if transcription:
            print(f"Original Transcription in Kannada: {transcription}")

            # Translate transcription from Kannada to English
            ...
            print(f"Translated Text (Kannada to English): {translated_text}")
        else:
        ...
        if translated_text:
            ...
            print(f"LLM Response Translated Back to Kannada: {translated_text_ta}")

            # Pass the Kannada response to TTS
            text_to_speech(translated_text_ta)
        else:

...
import textwrap
from utility import reprocess_audio_util
...
def text_to_speech(text, output_filename :str, speaker_id='EN-US', max_chunk_length=100):
    """
    ...
    Parameters:
        ...
        output_filename (str): Output audio file name.
    ...
    # Save the audio file
    sf.write(full_output_path, final_audio, 16000)  # Assuming 16kHz sample rate for the MMS-EN model
    print(f"Generated audio saved at: {full_output_path}")

    return full_output_path  # Return the full path to the saved file
...
def speech_to_speech_pipeline(input_audio, unique_filename :str, speaker_id="EN-US"):
    ...
    Parameters:
        ...
        unique_filename: Helps to name the output audio file.
    ...
    preprocessed_audio = reprocess_audio_util(input_filename=input_audio)

    # Step 1: ASR (Speech to Text)
    text = speech_to_text(preprocessed_audio)
...
def main(path :str,unique_filename :str):
    ...
    output_audio_file = speech_to_speech_pipeline(input_audio=path.replace("/","\\"), speaker_id="EN-US", unique_filename=unique_filename)
...
if __name__ == "__main__":
    input_audio_path = r"C:\Users\Ryan\Documents\Pinto Projeccts\Dail-Sathi\audio\input_audio\english.wav"  # Replace with actual audio path
    unique_filename = "output_eng1_filename"
    main(input_audio_path, unique_filename)

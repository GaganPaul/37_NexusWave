...
def hindi_text_to_speech(hindi_text, output_filename="indic_tts_out.wav"):
    """
    Convert a long Hindi text into speech using the TTS model.
    ...
    description_input_ids = DESCRIPTION_TOKENIZER(
        ...
        truncation=True,
        padding="longest",
        max_length=512  # Explicitly define the max length
    ).to(DEVICE)

    # Split the Hindi text into manageable chunks
    max_chunk_length = 20  # Adjust this based on the model's input capacity
    ...
    # Save the audio file
    sf.write(output_filename, final_audio, TTS_MODEL.config.sampling_rate)
